...
def rotate_gt_bbox(origin_gt_roidb, angle, gt_inds):
    ...
    for idx in range(len_of_gt):
        ...
        #step 3: normalize rotated_angle       (-90,90)
        cos_angle = np.cos(rotated_angle/180*np.pi)
        sin_angle = np.sin(rotated_angle/180*np.pi)
        final_anle = np.arctan(sin_angle/cos_angle)/np.pi*180
        

        rotated_gt_boxes[idx,0] = origin_gt_boxes[idx,0]
        rotated_gt_boxes[idx,1] = origin_gt_boxes[idx,1]
        rotated_gt_boxes[idx,2] = height * cfg.TRAIN.GT_MARGIN + cfg.TRAIN.GT_PADDING
        rotated_gt_boxes[idx,3] = width * cfg.TRAIN.GT_MARGIN + cfg.TRAIN.GT_PADDING
        rotated_gt_boxes[idx,4] = final_anle
