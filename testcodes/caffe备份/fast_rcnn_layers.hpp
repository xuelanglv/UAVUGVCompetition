...
protected:
	...
	int pooled_width_;
	Dtype spatial_scale_;
	Blob<Dtype> continuous_idx_x;
	Blob<Dtype> continuous_idx_y;
};
...
template<typename Dtype>
class EllipseONPoolingLayer: public Layer<Dtype> {
public:
	explicit EllipseONPoolingLayer(const LayerParameter& param) :
			Layer<Dtype>(param) {
	}
	virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);
...
protected:
	...
	vector<int> valid_x;
	vector<int> valid_y;
	void GetEllipseAreaON(int h, int w);
