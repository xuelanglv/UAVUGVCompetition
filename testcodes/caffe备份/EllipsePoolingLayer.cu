...
// nthreads: top[0]->count(); roi num * channels * w * h
// bottom_data: the pointer of feature maps
// bottom_rois: the pointer of proposal rois
template <typename Dtype>
__global__ void RotateROIPoolForward(const int nthreads, const Dtype* bottom_data, int ellipse_num,
    const Dtype spatial_scale, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width,
    const Dtype* bottom_rois, Dtype* top_data, int* argmax_data,const Dtype* info,
    int* valid_row, int* valid_col) {
    
    // Remove 0.5 shift for height & width
    int imageWidth = int(info[1]*spatial_scale);
    int imageHeight = int(info[0]*spatial_scale);
  CUDA_KERNEL_LOOP(index, nthreads) {
    // (n, c, 0, loc_idx) is an element in the pooled output
    int pw, ph, loc_idx, c, n;
    loc_idx = index % ellipse_num;
    ph = 0;
    c = (index / ellipse_num) % channels;
    n = index / ellipse_num / channels;
    
    // Here add the codes that convert the out feature point to the ellipse
    // note: ph == 0 (always)
    

    bottom_rois += n * 6;
    
    // (cx,cy, h, w, angle) the proposal ellipse
    ...
    Dtype P[8]; // Get the feature range of the roipooling feature (pw, ph)
    pw = valid_col[loc_idx], ph = valid_row[loc_idx];
    ...
    for (int h = topMost; h < bottomMost+1; ++h) 
    {
      for (int w = leftMost; w < rightMost+1; ++w) 
      {
           ...
           if(ABAB>ABAP&&ABAP>=0&&ACAC>ACAP&&ACAP>=0) // confirm the point (w,h) is in the rotated feature map
           {
...
template <typename Dtype>
void EllipseONPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  RotateROIPoolForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, bottom_data, valid_num, spatial_scale_, channels_, height_, width_,
      pooled_height_, pooled_width_, bottom_rois, top_data, argmax_data,image_info,
      valid_x.data(), valid_y.data());
...
template <typename Dtype>
__global__ void RotateROIPoolBackward(const int nthreads, const Dtype* top_diff, int ellipse_num,
    const int* argmax_data, const int num_rois, const Dtype spatial_scale,
    const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, Dtype* bottom_diff,
    const Dtype* bottom_rois) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    int c = (index / ellipse_num) % channels;
    int n = index / ellipse_num / channels;
...
template <typename Dtype>
void EllipseONPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  RotateROIPoolBackward<Dtype><<<CAFFE_GET_BLOCKS(counter), CAFFE_CUDA_NUM_THREADS>>>(
     count, top_diff, valid_num, argmax_data, top[0]->num(), spatial_scale_, channels_,
      height_, width_, pooled_height_, pooled_width_, bottom_diff, bottom_rois);
  CUDA_POST_KERNEL_CHECK;
  //std::cout<<top_diff[0]<<std::endl;
}

INSTANTIATE_LAYER_GPU_FUNCS(EllipseONPoolingLayer);
